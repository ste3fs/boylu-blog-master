      ...
      <section class="form-panel">
        <div class="panel-frame">
          ...
          <div class="panel-toolbar">
            <button class="icon-btn" type="button" @click="backToHome">
...
export default {
  ...
  computed: {
    ...
    panelSubtitle() {
      const map = {
        account: "请输入你的账号信息，继续进入站点。",
        login: "扫码关注公众号并发送登录码完成登录。",
        register: "注册后即可使用站点的完整登录能力。",
        forgot: "通过邮箱验证码重置你的登录密码。",
      };
  ...
  created() {
    ...
    if (rememberedEnabled && rememberedUsername && rememberedPassword) {
  ...
  methods: {
    persistRememberMe() {
      if (this.rememberMe && this.loginForm.username && this.loginForm.password) {
        setCookieExpires("remember_me", "1", 30);
        setCookieExpires("remember_username", this.loginForm.username, 30);
        setCookieExpires("remember_password", this.loginForm.password, 30);
        return;
      }
...
.login-shell {
  min-height: 100vh;
  display: grid;
  grid-template-columns: minmax(0, 1.32fr) minmax(720px, 0.98fr);
}
...
.panel-frame {
  ...
  box-shadow: 0 28px 90px rgba(76, 29, 149, 0.14);
